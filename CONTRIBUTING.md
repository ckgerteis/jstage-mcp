...
```bash
git clone https://github.com/ckgerteis/jstage-mcp
cd jstage-mcp
python -m venv .venv
...
`python mcpb/build.py` writes `dist/jstage-mcp-<version>.mcpb`. The bundle vendors nothing: its manifest
...
the install step and download everything inside the launch timeout, which is the fault 3.1.2 repaired.
